...
class CrosswordCreator():
    ...
    def const_aid(self, x, y, assignment):
        overlap = self.crossword.overlaps[x, y]
        if overlap == None:
            return True
    ...
    def consistent(self, assignment):
        ...
        for x in assignment:
            for y in assignment:
                if x != y:
                    if not self.const_aid(x, y, assignment):
                        return False
    ...
    def select_unassigned_variable(self, assignment):
        ...
        num_rem = [len(self.domains[var]) for var in variable]
        deg = [len(self.crossword.neighbors(var)) for var in variable]
        temp = list(zip(variable, num_rem, deg))
        def cmp(x, y):
            if x[1] != y[1]:
                if x[1] > y[1]:
                    return 1
        ...
        sorted(temp, key=functools.cmp_to_key(cmp))
